...
Let's consider now the **actual rule: sets can have up to 2 bottles**. For a single bottle $L_i$ with with a capacity $C_i$, there are three possible cases when we choose a fixed value $C$ for all sets:
...
A better version of the previous algorithm is one that analyzes only possible capacity values. **My code** takes into account **single-bottle sets**, as described before to make a first guess, but also **two-bottle sets**.
